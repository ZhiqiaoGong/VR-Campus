...
namespace RenderHeads.Media.AVProVideo
{
	...
	public class MediaPlayer : MonoBehaviour
	{
        ...
        /// <summary>
        /// 设置视频播放到某一帧，Seek(time)中time/1000即为视频的秒数
        /// </summary>
        public void playTime()//***********************************************************************************************************************************************************************************************************************************************************
        {
            lg = GameObject.Find("LgGraph").GetComponent<LgGraphOutput>();
            //获取位置逻辑图中的位置信息并计算视频帧数


            float pos = lg.GetPosition().location;  //0.0~1.0，0在表示起点
            float length = lg.GetPosition().locatedPath.length;//路径长度
#if TEST
			//测试用
			pos = Random.value;
			length = 43.0f;
#endif
            float p = pos * length * 1000;
			m_Control.Seek(p);
			//m_Control.SeekFast(p);
			m_Control.Pause();
		}

		public bool OpenVideoFromFile(FileLocation location, string path, bool autoPlay = true)
		{
            //path = "E:/校园漫游视频/2_3.mp4";
            Debug.Log(path);
		...
		protected virtual void Update()
		{
			...
			if (m_Control != null)
			{
				...
				//string baseUrl = "video/";
				//string nPath = baseUrl + lg.GetPosition().locatedPath.vUrl;//路径对应的视频的路径
				//if (!m_VideoPath.Equals(nPath))
				//{
				//	m_VideoPath = nPath;
				//	OpenVideoFromFile();
				//}

				//if (Time.frameCount % 5 == 0)//每5帧执行一次,待试验效果
					playTime();
